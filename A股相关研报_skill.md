...
name: A股相关研报_skill
description: >
  每日自动抓取宏观/策略/行业研报，聚焦A股市场，
  总结研报核心观点，提取相关标的（含行业龙头股票和ETF，仅限A股可买标的），
  结合研报逻辑给出投资建议，并参考YouTube频道研报分析视频结论。
version: "1.0"
focus: "A股（含科创板、创业板）"
---

## 一、每日定时任务（cron: 0 30 8 * * *，每天 08:30 执行）

执行顺序：
1. 拉取下方所有 RSS 源，过去 17 小时新内容
2. 过滤：只保留涉及 A股、中国宏观、人民币 的条目
3. 对每篇研报/文章生成结构化摘要（见第八节 Prompt 模板）
4. 提取相关标的：行业龙头个股（仅A股代码）+ ETF（仅A股上市ETF）
5. 结合研报逻辑输出投资建议
6. 参考 YouTube 频道 @oldpowerful 最新视频结论（见第十节）
7. 推送 Markdown 格式日报到 Telegram
...
每周一 8:30 用 Fetch 工具抓取以下页面，提取最新研报标题和摘要：
...
/memory set report_push_time "08:30"
...
## 八、完整每日 Prompt（复制进 OpenClaw 任务直接使用）

```
你是我的A股投研助手，专注中国A股资本市场（含沪市、深市、科创板、创业板）。每天早上08:30执行以下任务：
...
获取最近3条视频的标题和链接，逐一访问视频页面提取描述文字或字幕摘要。
将该频道的核心结论/研报观点纳入当日分析，标注"（来源：@oldpowerful）"。
...
- 重点关注行业：{行业1}、{行业2}、{行业3}
- 今日值得关注的标的（仅A股可买）：
  - 龙头个股：（列举3-5个，注明逻辑，只列A股代码）
  - 相关ETF：（列举2-3个，注明逻辑）

### 五、相关标的参考清单（仅A股）
（汇总今日研报涉及的所有A股标的，分为"龙头个股"和"ETF"两栏）

【Step 6 - 推送】
通过 Telegram Bot 推送到「每日投研」频道
```
...
**抓取方式：**
每日执行时，用 Fetch 工具访问该频道的视频列表页面，
提取最新3条视频的标题和 URL，逐一抓取视频描述/字幕，
提取研报结论和标的观点，整合到当日日报第三节。
